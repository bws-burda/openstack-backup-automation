    ...
    async def _check_database_health(self) -> ComponentHealth:
        ...
        try:
            if self.state_manager:
                ...
                _ = self.state_manager.get_last_backup("health-check-test")
                ...
                conn = sqlite3.connect(
                    self.database_path, timeout=self.config.timeout_seconds
                )
                cursor = conn.cursor()
    ...
    async def _check_openstack_health(self) -> ComponentHealth:
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["unauthorized", "forbidden", "authentication"]
            ):
                status = HealthStatus.UNHEALTHY
