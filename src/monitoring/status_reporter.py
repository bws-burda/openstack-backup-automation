...
from .models import HealthStatus, SystemStatus
...
class StatusReporter:
    ...
    def generate_backup_summary(self, days: int = 7) -> Optional[Dict[str, Any]]:
        ...
        if not self.state_manager:
            self.logger.warning(
                "Cannot generate backup summary: no state manager available"
            )
            return None
    ...
    def send_health_alert(self, system_status: SystemStatus) -> bool:
        ...
        if not self.notification_service:
            self.logger.warning(
                "Cannot send health alert: no notification service available"
            )
            return False
        ...
        try:
            ...
            alert_message = (
                f"System health check failed: {system_status.overall_status.value}"
            )
            if unhealthy_components:
                alert_message += (
                    f" - Unhealthy: {[c.name for c in unhealthy_components]}"
                )

            health_exception = Exception(alert_message)

            return self.notification_service.send_error_notification(
                health_exception, context
            )
    ...
    def send_status_report(
        self, system_status: SystemStatus, include_backup_summary: bool = True
    ) -> bool:
        ...
        if not self.notification_service:
            self.logger.warning(
                "Cannot send status report: no notification service available"
            )
            return False
        ...
        try:
            ...
            _ = self._format_report_as_text(health_report)
        ...
        for component in system_status.components:
            if component.name == "database" and component.is_unhealthy():
                ...
                recommendations.append(
                    "Verify OpenStack credentials and network connectivity"
                )
                ...
                recommendations.append(
                    "Check OpenStack quotas and storage service availability"
                )
            elif component.is_degraded():
                recommendations.append(
                    f"Monitor {component.name} component for potential issues"
                )
    ...
    def _format_report_as_text(self, report: Dict[str, Any]) -> str:
        ...
        for component in report["components"]:
            ...
            lines.append(
                f"{status_symbol} {component['name']}: {component['status']} - {component['message']}"
            )
