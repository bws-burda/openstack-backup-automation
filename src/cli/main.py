...
from ..logging.config import LoggingConfig, setup_logging as setup_comprehensive_logging
...
def run(ctx, dry_run, daemon, status):
    ...
    if not os.path.exists(config_path):
        ...
        click.echo("Copy config.yaml.example to config.yaml and adjust the values for your environment.")
        sys.exit(1)
    ...
    try:
        ...
        if status:
            ...
            async def show_status():
                ...
                click.echo(f"Resources due for backup: {info['resources_due_for_backup']}")
                click.echo(f"Last backup time: {info['last_backup_time'] or 'Never'}")
                click.echo(f"System status: {'Healthy' if info['healthy'] else 'Issues detected'}")
            ...
            runner = DaemonRunner(coordinator, config_obj.scheduling.check_interval_minutes)
            click.echo(f"Starting daemon mode (checking every {config_obj.scheduling.check_interval_minutes} minutes)")
            ...
            async def run_backup():
                ...
                click.echo(f"Backup cycle completed:")
...
def health(ctx, output_json, component, export):
    ...
    try:
        from ..factory import create_health_checker_from_config, create_status_reporter_from_config
        ...
        async def check_health():
            ...
            if component:
                ...
                if output_json:
                    ...
                    status_symbol = "✓" if component_health.is_healthy() else "⚠" if component_health.is_degraded() else "✗"
                    click.echo(f"{status_symbol} {component_health.name}: {component_health.status.value}")
                ...
                if output_json:
                    ...
                    overall_symbol = "✓" if system_status.overall_status.value == "healthy" else "⚠" if system_status.overall_status.value == "degraded" else "✗"
                    click.echo(f"=== System Health Status ===")
                    click.echo(f"{overall_symbol} Overall Status: {system_status.overall_status.value.upper()}")
...
"""
    ...
    try:
        ...
        if success:
            ...
            click.echo("Configuration file is valid but OpenStack connection failed", err=True)
            sys.exit(1)
...
@click.option("--user", default="backup",
              help="System user for the service (default: backup)")
@click.option("--config-dir", default="/etc/backup-automation",
              help="Configuration directory (default: /etc/backup-automation)")
@click.option("--data-dir", default="/var/lib/backup-automation",
...
def install(ctx, systemd, cron, user, config_dir, data_dir):
    ...
    if os.geteuid() != 0:
        click.echo("Error: Installation requires root privileges. Please run with sudo.", err=True)
        sys.exit(1)
    ...
    try:
        ...
        click.echo(f"2. Ensure the configuration is owned by {user}:root with 640 permissions")
        if systemd:
            click.echo(f"3. Enable and start the service: systemctl enable --now backup-automation.timer")
        if cron:
            click.echo(f"3. The cron job has been installed and will run every 15 minutes")
...
def install_cron_job(user, config_dir, data_dir):
    ...
    cron_content = f"""# OpenStack Backup Automation - runs every 15 minutes
*/15 * * * * cd {data_dir} && CONFIG_FILE={config_dir}/config.yaml /usr/local/bin/openstack-backup-automation run >/dev/null 2>&1
"""
    
    cron_file = f"/etc/cron.d/backup-automation"
    with open(cron_file, 'w') as f:
        f.write(f"# OpenStack Backup Automation\n")
        f.write(f"*/15 * * * * {user} cd {data_dir} && CONFIG_FILE={config_dir}/config.yaml /usr/local/bin/openstack-backup-automation run >/dev/null 2>&1\n")
