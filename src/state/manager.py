...
class StateManager(StateManagerInterface):
    ...
    def get_backup_statistics(self) -> dict:
        ...
        with self._get_connection() as conn:
            ...
            stats['verified_backups'] = verification_stats.get(1, 0)  # SQLite uses 1 for True
            stats['unverified_backups'] = verification_stats.get(0, 0)  # SQLite uses 0 for False
            
            # Total storage used (if size information is available)
            cursor = conn.execute("SELECT SUM(size_bytes) FROM backups WHERE size_bytes IS NOT NULL")
