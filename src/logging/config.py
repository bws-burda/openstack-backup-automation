...
class LoggingConfig:
    ...
    disabled_loggers: List[str] = field(default_factory=lambda: [
    ...
    def _validate(self):
        ...
        if self.format_type not in valid_formats:
            raise ValueError(f"Invalid format type: {self.format_type}. Must be one of: {valid_formats}")
...
def _create_file_handler(config: LoggingConfig, formatter: logging.Formatter) -> logging.Handler:
...
def _create_syslog_handler(config: LoggingConfig, formatter: logging.Formatter) -> Optional[logging.Handler]:
...
def get_logger(name: str, context: Optional[Dict[str, Union[str, int, float]]] = None) -> logging.Logger:
...
class ContextLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(self, logger: logging.Logger, context: Dict[str, Union[str, int, float]]):
