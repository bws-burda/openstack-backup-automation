...
class BackupChainManager:
    ...
    def get_chain_roots(self, resource_id: str) -> List[BackupInfo]:
        ...
        for backup in backup_chain:
            if (
                ...
                and backup.parent_backup_id is None
            ):
                roots.append(backup)
    ...
    def find_orphaned_backups(self, resource_id: str) -> List[BackupInfo]:
        ...
        orphaned = []
        for backup in backup_chain:
            if backup.parent_backup_id and backup.parent_backup_id not in existing_ids:
    ...
    def validate_chain_structure(self, resource_id: str) -> Dict[str, any]:
        ...
        if orphaned:
            ...
            validation_result["errors"].append(
                f"Found {len(orphaned)} orphaned backups"
            )
        ...
        if circular_refs:
            ...
            validation_result["errors"].append(
                f"Found circular references: {circular_refs}"
            )
    ...
    def get_chain_statistics(self, resource_id: str) -> Dict[str, any]:
        ...
        stats = {
            ...
            "full_backups": sum(
                1 for b in backup_chain if b.backup_type == BackupType.FULL
            ),
            "incremental_backups": sum(
                1 for b in backup_chain if b.backup_type == BackupType.INCREMENTAL
            ),
        ...
        for root in roots:
            ...
            chain_info = {
                ...
                "latest_backup": max(
                    [root] + descendants, key=lambda b: b.created_at or datetime.min
                ).created_at,
    ...
    def _validate_single_chain(
        self, root_backup_id: str, resource_id: str
    ) -> List[str]:
    ...
    def _has_full_backup_ancestor(
        self, backup: BackupInfo, backup_chain: List[BackupInfo]
    ) -> bool:
