...
class BackupStrategy:
    ...
    def determine_backup_type(
        ...
        if not last_full_backup:
            self.logger.info(
                f"No full backup found for resource {resource_id}, creating full backup"
            )
    ...
    def validate_backup_chain_integrity(self, resource_id: str) -> bool:
        ...
        if not validation_result["valid"]:
            for error in validation_result["errors"]:
                self.logger.error(
                    f"Chain validation error for resource {resource_id}: {error}"
                )
    ...
    def calculate_next_full_backup_date(self, resource_id: str) -> Optional[datetime]:
        ...
        return last_full_backup.created_at + timedelta(
            days=self.full_backup_interval_days
        )
    ...
    def should_create_backup(self, resource_id: str, backup_type: BackupType) -> bool:
        ...
        if backup_type == BackupType.FULL:
            last_full_backup = self.state_manager.get_last_full_backup(resource_id)
            return last_full_backup is None or self._is_full_backup_due(
                last_full_backup
            )
