...
class OperationVerifier:
    ...
    async def verify_backup_success(
        self, backup_info: BackupInfo, timeout_minutes: int = 60
    ) -> bool:
        """
        ...
        while True:
            try:
                ...
                if status is None:
                    self.logger.error(
                        f"Backup {backup_info.backup_id} not found during verification"
                    )
                    return False
                ...
                if status_lower in ["available", "completed", "active"]:
                    self.logger.info(
                        f"Backup {backup_info.backup_id} verified as successful"
                    )
                    return True
                ...
                if status_lower in ["error", "failed", "deleted"]:
                    self.logger.error(
                        f"Backup {backup_info.backup_id} failed with status: {status}"
                    )
                    return False
                ...
                self.logger.debug(
                    f"Backup {backup_info.backup_id} status: {status}, waiting..."
                )
    ...
    async def verify_operation_results(
        self, results: List[OperationResult], timeout_minutes: int = 60
    ...
    async def report_operation_failures(
        ...
        try:
            ...
            success = self.notification_service.send_backup_report(
                successful_operations=[], failed_operations=failed_results
            )
