...
class ErrorHandler:
    ...
    def __init__(self, notification_service: Optional[NotificationServiceInterface] = None):
    ...
    def categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(keyword in error_str for keyword in ["auth", "credential", "unauthorized", "401"]):
            return ErrorCategory.AUTHENTICATION
        
        # Network errors
        if any(keyword in error_str for keyword in ["network", "connection", "dns", "resolve"]) or \
           any(keyword in error_type for keyword in ["connection", "socket", "timeout"]):
            return ErrorCategory.NETWORK
        
        # Quota/limit errors
        if any(keyword in error_str for keyword in ["quota", "limit", "exceeded", "413"]):
        ...
        if any(keyword in error_type for keyword in ["os", "io", "file", "database", "sqlite"]):
    ...
    async def handle_error(
        ...
        if severity in [ErrorSeverity.CRITICAL, ErrorSeverity.HIGH] and self.notification_service:
            ...
            try:
                self.notification_service.send_error_notification(error, notification_context)
