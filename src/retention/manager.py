...
from ..interfaces import (OpenStackClientInterface, RetentionManagerInterface,
...
class RetentionManager(RetentionManagerInterface):
    ...
    def get_backups_to_delete(
        ...
        """
        self.logger.info(f"Evaluating backups for deletion with {retention_policy.retention_days} day retention")
        ...
        self.logger.info(f"Found {len(old_backups)} backups older than {retention_policy.retention_days} days")
        ...
        for resource_id, resource_backups in backups_by_resource.items():
            self.logger.debug(f"Processing {len(resource_backups)} old backups for resource {resource_id}")
            ...
            self.logger.debug(f"Resource {resource_id}: {len(deletable_backups)} backups marked for deletion")
    ...
    def _apply_retention_rules(
        ...
        self.logger.debug(f"Applying retention rules for resource {resource_id}: "
        ...
        for backup in old_backups:
            ...
            if remaining_after_deletion < 1:
                can_delete = False
                skip_reason = f"Would leave only {remaining_after_deletion} backups (minimum: 1)"
            ...
            # Rule 4: For incremental backups, check if parent still exists
                ...
                if backup.parent_backup_id:
                    ...
                    if not parent_exists:
                        ...
                        self.logger.debug(f"Incremental backup {backup.backup_id} is orphaned (parent missing)")

            if can_delete:
                deletable.append(backup)
                self.logger.debug(f"Backup {backup.backup_id} ({backup.backup_type.value}) marked for deletion")
            else:
                self.logger.debug(f"Backup {backup.backup_id} ({backup.backup_type.value}) protected: {skip_reason}")
    ...
    async def delete_backup(self, backup_info: BackupInfo) -> bool:
        ...
        """
        try:
            self.logger.info(f"Deleting backup {backup_info.backup_id} ({backup_info.backup_type.value}) "
            ...
            if success:
                ...
                self.logger.error(f"Failed to delete backup {backup_info.backup_id} from OpenStack")
                return False

        except Exception as e:
            self.logger.error(f"Exception while deleting backup {backup_info.backup_id}: {e}")
            return False
    ...
    async def cleanup_expired_backups(
        ...
        cleanup_result = {
            ...
            "failed_count": 0,
            "space_freed_bytes": 0,
            "policies_applied": list(retention_policies.keys()) if retention_policies else ["default"],
            "batch_results": None,
            "chain_deletions": []
        }
        ...
        if not retention_policies:
            retention_policies = {
                "default": RetentionPolicy(retention_days=30)
            }

        default_policy = retention_policies.get("default", next(iter(retention_policies.values())))
        ...
        for backup in backups_to_delete:
            if backup.backup_type == BackupType.FULL:
                dependent_incrementals = self.state_manager.get_dependent_incrementals(backup.backup_id)
                if dependent_incrementals:
                    # This full backup has dependents - handle specially
                    full_backups_with_dependents.append((backup, dependent_incrementals))
                else:
        ...
        for full_backup, dependents in full_backups_with_dependents:
            try:
                ...
                if use_batch_deletion and len(dependents) > 1:
                    ...
                    batch_result = await self.delete_backups_batch(dependents, batch_size)
                    chain_result["dependents_deleted"] = len(batch_result["successful_deletions"])
                    cleanup_result["deleted_count"] += chain_result["dependents_deleted"]
                    ...
                    if batch_result["failed_deletions"]:
                        chain_result["errors"].extend([f["error"] for f in batch_result["failed_deletions"]])
                else:
                    ...
                    for incremental in dependents:
                        if await self.delete_backup(incremental):
                            ...
                            cleanup_result["space_freed_bytes"] += incremental.size_bytes or 0
                        else:
                            cleanup_result["failed_count"] += 1
                            chain_result["errors"].append(f"Failed to delete dependent {incremental.backup_id}")
                ...
                if await self.delete_backup(full_backup):
                    ...
                    cleanup_result["failed_count"] += 1
                    chain_result["errors"].append(f"Failed to delete full backup {full_backup.backup_id}")

                cleanup_result["chain_deletions"].append(chain_result)

            except Exception as e:
                self.logger.error(f"Exception while processing full backup chain {full_backup.backup_id}: {e}")
        ...
        if standalone_backups:
            if use_batch_deletion:
                self.logger.info(f"Using batch deletion for {len(standalone_backups)} standalone backups")
                batch_result = await self.delete_backups_batch(standalone_backups, batch_size)
                cleanup_result["batch_results"] = batch_result
                cleanup_result["deleted_count"] += len(batch_result["successful_deletions"])
                cleanup_result["failed_count"] += len(batch_result["failed_deletions"])
                cleanup_result["space_freed_bytes"] += batch_result["space_freed_bytes"]
            else:
                self.logger.info(f"Using sequential deletion for {len(standalone_backups)} standalone backups")
                for backup in standalone_backups:
                    try:
                        if await self.delete_backup(backup):
                            cleanup_result["deleted_count"] += 1
                            cleanup_result["space_freed_bytes"] += backup.size_bytes or 0
                        else:
                            cleanup_result["failed_count"] += 1
                    except Exception as e:
                        self.logger.error(f"Exception while deleting backup {backup.backup_id}: {e}")
                        cleanup_result["failed_count"] += 1

        self.logger.info(f"Cleanup completed: {cleanup_result['deleted_count']} deleted, "
    ...
    def get_retention_candidates(
        ...
        for policy_name, policy in retention_policies.items():
            ...
            if resource_filter:
                policy_candidates = [
                    backup for backup in policy_candidates 
                    if backup.resource_id == resource_filter
                ]
            
            candidates[policy_name] = policy_candidates
            self.logger.debug(f"Policy '{policy_name}': {len(policy_candidates)} candidates")
    ...
    def validate_deletion_safety(self, backup_info: BackupInfo) -> Dict[str, any]:
        ...
        # Check for dependent incremental backups
        if backup_info.backup_type == BackupType.FULL:
            dependents = self.state_manager.get_dependent_incrementals(backup_info.backup_id)
            if dependents:
        ...
        if backup_info.backup_type == BackupType.FULL:
            ...
            if len(full_backups) == 1 and full_backups[0].backup_id == backup_info.backup_id:
                safety_result["warnings"].append("This is the last full backup for the resource")
    ...
    def get_backup_chain_summary(self, resource_id: str) -> Dict[str, any]:
        ...
        for full_backup in full_backups:
            dependents = self.state_manager.get_dependent_incrementals(full_backup.backup_id)
            chain_info = {
                "root_backup_id": full_backup.backup_id,
                "root_created_at": full_backup.created_at,
                "incremental_count": len(dependents),
                "chain_size_bytes": (full_backup.size_bytes or 0) + sum(d.size_bytes or 0 for d in dependents),
                "all_verified": full_backup.verified and all(d.verified for d in dependents)
            }
    ...
    def schedule_cleanup_operation(
        ...
        self.logger.info(f"Cleanup operation planned: {len(operation_plan['safe_deletions'])} safe deletions, "
    ...
    async def delete_backups_batch(
        ...
        batches = [backups[i:i + batch_size] for i in range(0, len(backups), batch_size)]
        batch_result["total_batches"] = len(batches)
        
        self.logger.info(f"Starting batch deletion of {len(backups)} backups in {len(batches)} batches")
        
        for batch_idx, batch in enumerate(batches):
            self.logger.debug(f"Processing batch {batch_idx + 1}/{len(batches)} with {len(batch)} backups")
            ...
            try:
                ...
                for backup, result in zip(batch, batch_results):
                    if isinstance(result, Exception):
                        ...
                        self.logger.error(f"Batch deletion failed for {backup.backup_id}: {result}")
                    elif result:
                        ...
                        self.logger.debug(f"Successfully deleted backup {backup.backup_id}")
                    else:
                        batch_result["failed_deletions"].append({
                            "backup_id": backup.backup_id,
                            "backup_type": backup.backup_type.value,
                        ...
                        self.logger.error(f"Batch deletion returned False for {backup.backup_id}")
                
            except Exception as e:
                self.logger.error(f"Exception during batch {batch_idx + 1} execution: {e}")
        ...
        self.logger.info(f"Batch deletion completed: {success_count} successful, {failed_count} failed, "
    ...
    def should_create_new_full_backup(
        ...
        resource_id: str, 
        schedule_tag: str,
        default_full_backup_interval: int = 7
    ) -> bool:
        ...
        if should_create:
            self.logger.info(f"Resource {resource_id}: {days_since_last_full} days since last full backup "
                           f"(interval: {full_backup_interval} days) - new full backup needed")
        else:
            self.logger.debug(f"Resource {resource_id}: {days_since_last_full} days since last full backup "
    ...
    def get_backup_strategy_for_resource(
        ...
        if last_full_backup.created_at:
            ...
            if days_since_last_full >= strategy["full_backup_interval_days"]:
                strategy["should_create_full_backup"] = True
                strategy["backup_type_recommended"] = BackupType.FULL
                strategy["reasoning"] = f"Last full backup is {days_since_last_full} days old (interval: {strategy['full_backup_interval_days']} days)"
            else:
                strategy["reasoning"] = f"Last full backup is {days_since_last_full} days old (interval: {strategy['full_backup_interval_days']} days) - incremental sufficient"
        else:
            strategy["should_create_full_backup"] = True
            strategy["backup_type_recommended"] = BackupType.FULL
    ...
    def create_retention_policy_from_tag(
        ...
        return RetentionPolicy(
            retention_days=tag_retention_info.get('retention_days', default_policy.retention_days),
    ...
    def get_backups_to_delete_with_tag_policies(
        ...
        for resource_id, resource_backups in backups_by_resource.items():
            self.logger.debug(f"Processing {len(resource_backups)} backups for resource {resource_id}")
            ...
            for backup in resource_backups:
                ...
                if self._is_backup_deletable_under_policy(backup, all_resource_backups, effective_policy):
                    backups_to_delete.append(backup)
                    self.logger.debug(f"Backup {backup.backup_id} marked for deletion "
                                    f"(policy: {effective_policy.retention_days}d retention)")
        
        self.logger.info(f"Total backups marked for deletion with tag policies: {len(backups_to_delete)}")
        return backups_to_delete
    ...
    def _get_effective_retention_policy(
        ...
        if global_policies:
            ...
            if backup.backup_type == BackupType.SNAPSHOT and 'snapshots' in global_policies:
                return global_policies['snapshots']
            elif backup.backup_type == BackupType.FULL and 'full_backups' in global_policies:
                return global_policies['full_backups']
            elif backup.backup_type == BackupType.INCREMENTAL and 'incremental_backups' in global_policies:
        ...
        # Rule 3: For incremental backups, check parent chain integrity
            ...
            if backup.parent_backup_id:
                ...
                dependent_incrementals = [
                    ...
                    if b.backup_type == BackupType.INCREMENTAL and b.parent_backup_id == backup.backup_id
                ]
                if dependent_incrementals:
                    return False  # Don't delete if other incrementals depend on this one
    ...
    def _ensure_chain_integrity_after_deletion(
        ...
        """
        # Simulate deletion
        remaining_backups = [b for b in all_backups if b.backup_id != backup_to_delete.backup_id]
        ...
        for backup in remaining_backups:
            if backup.backup_type == BackupType.INCREMENTAL and backup.parent_backup_id:
                ...
                if not parent_exists:
                    self.logger.warning(f"Deleting {backup_to_delete.backup_id} would orphan incremental {backup.backup_id}")
                    return False
        
        # Ensure at least one full backup remains if there are incrementals
        remaining_full_backups = [b for b in remaining_backups if b.backup_type == BackupType.FULL]
        remaining_incrementals = [b for b in remaining_backups if b.backup_type == BackupType.INCREMENTAL]
        
        if remaining_incrementals and not remaining_full_backups:
            self.logger.warning(f"Deleting {backup_to_delete.backup_id} would leave incrementals without any full backup")
            return False
    ...
    async def _delete_backup_with_result(self, backup_info: BackupInfo) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Exception in _delete_backup_with_result for {backup_info.backup_id}: {e}")
            raise  # Re-raise for gather() to catch

    async def delete_backup_chain_aware(self, backup_info: BackupInfo, force: bool = False) -> Dict[str, any]:
        ...
        if not safety_check["safe"] and not force:
            ...
            deletion_result["warnings"].append("Deletion aborted due to safety concerns (use force=True to override)")
            return deletion_result
        
        try:
            # If this is a full backup with dependents and force=True, delete dependents first
            if (backup_info.backup_type == BackupType.FULL and 
                safety_check["dependent_backups"] and force):
                
                self.logger.warning(f"Force deleting full backup {backup_info.backup_id} with {len(safety_check['dependent_backups'])} dependents")
                
                # Get all dependent backups
                dependents = self.state_manager.get_dependent_incrementals(backup_info.backup_id)
                
                # Sort dependents by creation date (newest first to avoid breaking chains)
                dependents.sort(key=lambda b: b.created_at or datetime.min, reverse=True)
                ...
                for dependent in dependents:
                    if await self.delete_backup(dependent):
                        ...
                        self.logger.info(f"Deleted dependent backup {dependent.backup_id}")
                    else:
                        ...
                        self.logger.error(f"Failed to delete dependent backup {dependent.backup_id}")
            ...
            self.logger.error(f"Exception during chain-aware deletion of {backup_info.backup_id}: {e}")
            deletion_result["warnings"].append(f"Exception during deletion: {str(e)}")
        
        return deletion_result

    async def cleanup_backup_chain(self, resource_id: str, retention_policy: RetentionPolicy) -> Dict[str, any]:
        ...
        try:
            ...
            self.logger.info(f"Starting chain cleanup for resource {resource_id} with {len(all_backups)} backups")
            
            # Get backups to delete based on retention policy
            old_backups = self.state_manager.get_backups_older_than(retention_policy.retention_days)
            resource_old_backups = [b for b in old_backups if b.resource_id == resource_id]
            ...
            chains_to_process = self._group_backups_by_chain(deletable_backups, all_backups)
            ...
            for chain_root, chain_backups in chains_to_process.items():
                ...
                # Merge results
                cleanup_result["deleted_backups"].extend(chain_result["deleted_backups"])
                cleanup_result["failed_deletions"].extend(chain_result["failed_deletions"])
                cleanup_result["warnings"].extend(chain_result["warnings"])
                cleanup_result["space_freed_bytes"] += chain_result.get("space_freed_bytes", 0)
                
                if not chain_result["success"]:
                    cleanup_result["success"] = False
            
            self.logger.info(f"Chain cleanup completed for resource {resource_id}: "
                           f"{len(cleanup_result['deleted_backups'])} deleted, "
                           f"{len(cleanup_result['failed_deletions'])} failed")
        
        except Exception as e:
            self.logger.error(f"Exception during chain cleanup for resource {resource_id}: {e}")
            cleanup_result["success"] = False
    ...
    def _group_backups_by_chain(
        self, 
        deletable_backups: List[BackupInfo], 
        all_backups: List[BackupInfo]
    ...
    def _find_chain_root(self, backup: BackupInfo, all_backups: List[BackupInfo]) -> Optional[str]:
    ...
    async def _cleanup_single_chain(
        ...
        for backup in chain_backups:
            try:
                ...
                deletion_result = await self.delete_backup_chain_aware(backup, force=False)
                
                if deletion_result["success"]:
                    chain_result["deleted_backups"].extend(deletion_result["deleted_backups"])
                    chain_result["space_freed_bytes"] += space_to_free
                else:
                    chain_result["failed_deletions"].extend(deletion_result["failed_deletions"])
                    chain_result["success"] = False
                
                chain_result["warnings"].extend(deletion_result["warnings"])
                
            except Exception as e:
                self.logger.error(f"Exception while cleaning up backup {backup.backup_id}: {e}")
    ...
    async def repair_broken_chains(self, resource_id: str, dry_run: bool = True) -> Dict[str, any]:
        ...
        try:
            ...
            for backup in all_backups:
                if (backup.backup_type == BackupType.INCREMENTAL and 
                    backup.parent_backup_id and 
                    backup.parent_backup_id not in backup_ids):
            ...
            self.logger.info(f"Found {len(orphaned)} orphaned backups for resource {resource_id}")
            ...
            if not dry_run:
                for orphaned_backup in orphaned:
                    if await self.delete_backup(orphaned_backup):
                        ...
                        self.logger.error(f"Failed to remove orphaned backup {orphaned_backup.backup_id}")
            
            # Analyze remaining chains
            remaining_backups = [b for b in all_backups if b not in orphaned] if not dry_run else all_backups
            chain_analysis = self._analyze_chain_structure(remaining_backups)
            repair_result["repaired_chains"] = chain_analysis
            
        except Exception as e:
            self.logger.error(f"Exception during chain repair for resource {resource_id}: {e}")
            repair_result["success"] = False
            repair_result["warnings"].append(f"Exception during repair: {str(e)}")
        
        return repair_result

    def _analyze_chain_structure(self, backups: List[BackupInfo]) -> List[Dict[str, any]]:
        ...
        for full_backup in full_backups:
            ...
            chain_info = {
                ...
                "total_size_bytes": (full_backup.size_bytes or 0) + sum(i.size_bytes or 0 for i in chain_incrementals)
            }
